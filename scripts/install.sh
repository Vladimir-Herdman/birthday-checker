...
main() {
    ...
    printf "%s\n" "  Copying binary to installation directory at: $INSTALL_DIR/$BINARY_NAME"
    cp -f ./bin/main "$INSTALL_DIR/$BINARY_NAME"

    if [[ ":$PATH:" == *":$INSTALL_DIR:"* ]]; then
        printf "%s\n" "  You may now call '$BINARY_NAME' to use this script"
    else
        ...
        if [[ "$current_shell" == *'zsh'* ]]; then
            ...
            printf "%s\n" "  You may now call '$BINARY_NAME' to use this script, but open a new shell instance first"
